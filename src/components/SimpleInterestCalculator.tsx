...
const SimpleInterestCalculator: React.FC<CalculatorProps> = ({
  principal,
  annualInterestRate,
  numPayments,
  paymentFrequency,
  ...
  useEffect(() => {
    if (paymentDates.length > 0) {
      calculateSimpleInterestPayments();
    }
  }, [paymentDates]);
  ...
  const calculateSimpleInterestPayments = () => {
    const totalInterest = (principal * annualInterestRate) / 100;
    const interestPerPayment = totalInterest / numPayments;
    const capitalPerPayment = principal / numPayments;

    const calculatedPayments: Payment[] = [];
    let remainingPrincipal = principal;

    paymentDates.forEach((date, index) => {
      const interest = remainingPrincipal * (annualInterestRate / 100);
      const totalToPay = capitalPerPayment + interest;
      ...
      calculatedPayments.push({
        ...
        capitalPayment: capitalPerPayment,
        interest,
      });

      remainingPrincipal -= capitalPerPayment;
    });
